...
@app.route('/index')
def home():
	return render_template('index.html', title = 'Help App - Homepage')
...
def help():
	...
	db.child('help').child(timestamp).set({
		'lat' : lat,
		'long' : lon
	})
...
def admin_panel():
	...
	print(help_data.val())
	print(pdna_data.val())
	return render_template('admin.html', help_data = help_data.val(), pdna_data = pdna_data.val())